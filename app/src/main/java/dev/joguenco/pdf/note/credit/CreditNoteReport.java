...
public class CreditNoteReport {
    ...
    public Boolean generateReport(
        ...
        try {
                    ...
                    JasperFillManager.fillReport(
                            ...
                            obtenerMapaParametrosReportes(
                                    getParametersInfoTriobutaria(
                                            rep.getNotaCredito().getInfoTributaria(),
                                            numAut,
                                            fechaAut),
            ...
            ReportUtil.savePdfReport(
                    reporte_view,
                    rep.getNotaCredito().getInfoTributaria().getClaveAcceso(),
                    pdfOutFolder);
    ...
    private Map<String, Object> getInfoCN(
            NotaCredito.InfoNotaCredito infoCN, CreditNoteTemplate nc) {
    ...
    private String ObtieneIvaRideNC(TotalConImpuestos impuestos, Date fecha) {
        for (TotalConImpuestos.TotalImpuesto impuesto : impuestos.getTotalImpuesto()) {
            ...
            if (TypeTaxEnum.IVA.getCode() == cod.intValue()
                    && impuesto.getValor().doubleValue() > 0.0D)
