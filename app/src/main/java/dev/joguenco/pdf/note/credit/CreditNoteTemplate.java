...
public class CreditNoteTemplate {
    ...
    public List<TotalReceipts> getTotalReceipts() {
        ...
        totalReceipts.add(
                new TotalReceipts(
                        "VALOR TOTAL",
                        this.notaCredito.getInfoNotaCredito().getValorModificacion(),
                        false));
    ...
    private TotalReceipt getTotalesNC(NotaCredito.InfoNotaCredito infoNc) {
        ...
        for (TotalConImpuestos.TotalImpuesto ti :
                infoNc.getTotalConImpuestos().getTotalImpuesto()) {
            Integer cod = Integer.parseInt(ti.getCodigo());
            var tarifa =
                    ti.getValor().divide(ti.getBaseImponible(), 2, java.math.RoundingMode.HALF_UP);
