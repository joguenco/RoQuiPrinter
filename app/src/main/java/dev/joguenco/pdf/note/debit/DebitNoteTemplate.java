...
public class DebitNoteTemplate {
    ...
    public List<AdditionalInformation> getInfoAdicional() {
        if (this.notaDebito.getInfoAdicional() != null && !this.notaDebito.getInfoAdicional().getCampoAdicional().isEmpty()) {
            ...
            for (NotaDebito.InfoAdicional.CampoAdicional info : this.notaDebito.getInfoAdicional().getCampoAdicional()) {
                AdditionalInformation ia = new AdditionalInformation(info.getValue(), info.getNombre());
    ...
    public List<PayMethod> getFormasPago() {
        if (getNotaDebito().getInfoNotaDebito().getPagos() != null) {
            ...
            if (getNotaDebito().getInfoNotaDebito().getPagos().getFirst().getPago() != null && !getNotaDebito().getInfoNotaDebito().getPagos().getFirst().getPago().isEmpty())
                for (var pa : getNotaDebito().getInfoNotaDebito().getPagos().getFirst().getPago())
                    this.payMethod.add(new PayMethod(getNamePayMethod(pa.getFormaPago()), pa.getTotal().setScale(2).toString()));
        }
    ...
    public List<TotalReceipts> getTotalesComprobante() {
        ...
        for (var iva : tc.getIvaDistintoCero()) {
            if (iva.getSubtotal().compareTo(BigDecimal.ZERO) > 0)
                totalesComprobante.add(new TotalReceipts("SUBTOTAL " + iva.getTarifa() + "%", iva.getSubtotal(), false));
        }
        ...
        if (tc.getSubtotalNoSujetoIva().compareTo(BigDecimal.ZERO) > 0)
            totalesComprobante.add(new TotalReceipts("SUBTOTAL NO OBJETO IVA", tc.getSubtotalNoSujetoIva(), false));
        if (tc.getSubtotalExentoIVA().compareTo(BigDecimal.ZERO) > 0)
            totalesComprobante.add(new TotalReceipts("SUBTOTAL EXENTO IVA", tc.getSubtotalExentoIVA(), false));
        totalesComprobante.add(new TotalReceipts("SUBTOTAL SIN IMPUESTOS", this.notaDebito.getInfoNotaDebito().getTotalSinImpuestos(), false));
        if (tc.getTotalIce().compareTo(BigDecimal.ZERO) > 0)
            totalesComprobante.add(new TotalReceipts("ICE", tc.getTotalIce(), false));
        for (var iva : tc.getIvaDistintoCero())
            totalesComprobante.add(new TotalReceipts("IVA " + iva.getTarifa() + "%", iva.getValor(), false));
        ...
        if (!compensaciones.equals(BigDecimal.ZERO.setScale(2))) {
            ...
            totalesComprobante.add(new TotalReceipts("VALOR A PAGAR", this.notaDebito.getInfoNotaDebito().getValorTotal(), false));
        } else {
            totalesComprobante.add(new TotalReceipts("VALOR TOTAL", this.notaDebito.getInfoNotaDebito().getValorTotal(), false));
        }
